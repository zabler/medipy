...
def preprocessing_old(samples, sample_rate):  # , ecg, plot=False):
    '''
    ...
    for i in range(0, len(samples), sample_rate):
        ...
        coeffs_denoised = cA3, cD3_thresh, cD2_thresh, cD1_thresh  # vs. cA3 trsh
        samples_denoised = pywt.waverec(coeffs_denoised, 'coif1')  # , mode='zero')
        #samples_filtered += list(samples_denoised)
    ...
    # # TK Energy Operator
    # samples_filtered_tk = []
    # for index, val in enumerate(samples_filtered[1:-1]):
    #     samples_filtered_tk.append((val * val) - (samples_filtered[index - 1] * samples_filtered[index + 1]))
    # samples_filtered_tk.insert(0, samples_filtered_tk[0])
    # samples_filtered_tk.append(samples_filtered_tk[-1])
...
def preprocessing(samples, sample_rate):  # , ecg, plot=False):
    ...
    for i in range(0, len(samples), sample_rate):
        ...
        # Wavelet Reconstruciton
        coeffs_denoised = cA3, cD3_thresh, cD2_thresh, cD1_thresh  # vs. cA3_thresh
        samples_denoised = pywt.waverec(coeffs_denoised, 'coif1')  # , mode='zero')
        #samples_filtered += list(samples_denoised)
        ...
        samples_filtered += samples_filtered_tk

    # samples_denoised = samples_filtered
    # # TK Energy Operator
    # samples_filtered_tk = []
    # for index, val in enumerate(samples_denoised[1:-1]):
    #     samples_filtered_tk.append((val * val) - (samples_denoised[index - 1] * samples_denoised[index + 1]))
